...
export default function SupportCenterPage() {
  ...
  const [messages, setMessages] = useState([
    {
      sender: "agent",
      text: "Hello! I am SpotBot, your operations assistant. How can I help coordinate your food truck slot today?",
      time: "10:00 AM",
    },
  ]);
  ...
  const handleSendChat = (e) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      let replyText =
        "Understood. Let me look up your current reservation coordinates and check compliance codes.";
      if (
        chatInput.toLowerCase().includes("permit") ||
        chatInput.toLowerCase().includes("zoning")
      ) {
        replyText =
          "SpotReserve handles city permits automatically. You can download active permit tags inside Dashboard > Upcoming Bookings.";
      } else if (
        chatInput.toLowerCase().includes("payout") ||
        chatInput.toLowerCase().includes("revenue")
      ) {
        replyText =
          "Your revenue payouts settle in Chase Checking. Processing takes up to 24 hours depending on your plan tier.";
      } else if (
        chatInput.toLowerCase().includes("power") ||
        chatInput.toLowerCase().includes("hookup")
      ) {
        replyText =
          "Pier 14 provides 50A utility grid hookups, SOMA Plaza provides 30A. Double check spec dimensions in Profile.";
      }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-sm sm:text-base text-brand-slate font-medium max-w-xl mx-auto">
            Create compliance support tickets or talk directly to our automated
            chatbot system for quick replies.
          </p>
        </div>
        ...
        {ticketSubmitted && (
          <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs font-bold flex items-center space-x-2 shadow-sm animate-pulse max-w-lg mx-auto">
            ...
            <span>
              Support ticket submitted! A compliance operator will reply to your
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch">
          ...
          <form
            ...
            className="md:col-span-5 p-6 rounded-3xl bg-white border border-brand-navy/5 shadow-sm flex flex-col justify-between"
          >
                ...
                <div className="space-y-1">
                  ...
                  >
                    <option value="Technical">
                      Technical (electrical, WiFi)
                    </option>
                    <option value="Billing">
                      Billing (payouts, transactions)
                    </option>
          ...
          <div className="md:col-span-7 rounded-3xl bg-white border border-brand-navy/5 shadow-xl flex flex-col h-[480px] overflow-hidden">
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <span className="block text-xs font-bold text-brand-navy">
                    SpotReserve Assistant
                  </span>
                  <span className="block text-[9px] text-brand-slate font-mono font-semibold">
                    BOT SUPPORT ACTIVE
                  </span>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-brand-cream/10">
              {messages.map((m, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex ${isAgent ? "justify-start" : "justify-end"} items-end space-x-2`}
                  >
                    {isAgent && (
                    ...
                    <div
                      className={`p-3 max-w-[75%] rounded-2xl text-xs leading-relaxed ${
                        isAgent
                    ...
                    >
                      <p>{m.text}</p>
                      <span
                        className={`block text-[8px] mt-1 text-right ${isAgent ? "text-brand-slate/60" : "text-white/60"}`}
                      >
                        {m.time}
                      </span>
                    </div>
            ...
            <form
              ...
              className="p-3 border-t border-brand-navy/5 bg-white flex items-center space-x-2 shrink-0"
            >
              <input
                type="text"
                value={chatInput}
