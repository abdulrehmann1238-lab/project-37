...
import { Bell, ShieldCheck, CreditCard, AlertTriangle, CheckCircle, Info } from "lucide-react";
...
export default function NotificationCenter() {
  ...
  const getIcon = (type: string) => {
    if (type === "booking") return <ShieldCheck className="w-5 h-5 text-brand-teal" />;
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-brand-navy">Notification Center</h1>
          <p className="text-xs text-brand-slate font-medium">Review real-time system alerts, booking permits status, and direct payout receipts.</p>
        </div>
          ...
          >
            ...
            <div className="space-y-1.5 flex-1">
              <div className="flex items-baseline space-x-2">
                <h3 className={`text-sm font-extrabold ${notif.unread ? "text-brand-navy font-bold" : "text-brand-slate"}`}>
                  {notif.title}
                </h3>
                <span className="text-[9px] text-brand-slate/60 font-mono font-bold">{notif.time}</span>
              </div>
