...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { name: "Upcoming Bookings", path: "/dashboard/upcoming", icon: <Calendar className="w-4 h-4" /> },
    { name: "Booking History", path: "/dashboard/history", icon: <History className="w-4 h-4" /> },
    { name: "Revenue Analytics", path: "/dashboard/revenue", icon: <TrendingUp className="w-4 h-4" /> },
    { name: "Availability Calendar", path: "/dashboard/calendar", icon: <Calendar className="w-4 h-4" /> },
    { name: "Notifications", path: "/dashboard/notifications", icon: <Bell className="w-4 h-4" /> },
    { name: "Truck Owner Profile", path: "/dashboard/profile", icon: <User className="w-4 h-4" /> },
    { name: "Team Management", path: "/dashboard/team", icon: <Users className="w-4 h-4" /> }
  ];

  const notifications = [
    { text: "Pier 14 booking confirmed for June 12.", time: "2 hours ago", unread: true },
    { text: "Payment of $195.00 completed successfully.", time: "2 hours ago", unread: false },
    { text: "Electrical Hookup maintenance scheduled at Soma Plaza.", time: "1 day ago", unread: false }
  ];
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 border-r border-brand-navy/5 bg-brand-cream/30 shrink-0 select-none">
        ...
        <div className="p-6 border-b border-brand-navy/5">
          <Link href="/" className="flex items-center space-x-2.5">
            ...
            <div>
              <span className="text-sm font-extrabold text-brand-navy block tracking-tight">SpotReserve</span>
              <span className="text-[9px] text-brand-teal font-extrabold uppercase font-mono tracking-widest block">Owner Portal</span>
            </div>
          </Link>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3">
                  <span className={isActive ? "text-brand-teal" : "text-brand-slate"}>
                    {item.icon}
                  </span>
                  <span>{item.name}</span>
                </div>
        ...
        <header className="h-16 border-b border-brand-navy/5 bg-white flex items-center justify-between px-6 shrink-0 relative z-20">
          ...
          <div className="flex items-center space-x-4">
              ...
              {notificationsOpen && (
                <div className="absolute right-0 mt-3 w-80 rounded-2xl bg-white border border-brand-navy/10 shadow-2xl p-4 space-y-3">
                  <div className="flex justify-between items-center pb-2 border-b border-brand-navy/5">
                    <span className="text-xs font-bold uppercase tracking-wider text-brand-navy">Notifications</span>
                    <Link href="/dashboard/notifications" onClick={() => setNotificationsOpen(false)} className="text-[10px] text-brand-teal hover:underline font-semibold font-mono">
                      View All
                    </Link>
                  </div>
                  <div className="space-y-2.5">
                    {notifications.map((notif, idx) => (
                      <div key={idx} className="text-xs space-y-0.5">
                        <p className={`font-semibold ${notif.unread ? "text-brand-navy" : "text-brand-slate"}`}>
                          {notif.text}
                        </p>
                        <span className="text-[9px] text-brand-slate/60 font-mono block">{notif.time}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <Link href="/dashboard/profile" className="flex items-center space-x-2">
              ...
              <span className="hidden sm:inline-block text-xs font-bold text-brand-navy">Fire & Slice</span>
            </Link>
          </div>
        </header>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between pb-4 border-b border-brand-navy/5">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <span className="text-sm font-extrabold text-brand-navy block">SpotReserve</span>
                    <span className="text-[9px] text-brand-teal font-extrabold block">Owner Portal</span>
                  </div>
                </div>
                <button
