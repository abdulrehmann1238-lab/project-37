...
import { Users, UserPlus, ShieldAlert, Mail, Trash2 } from "lucide-react";

export default function TeamManagement() {
  const [team, setTeam] = useState([
    { name: "Marcus Vance", email: "marcus@fireandslice.co", role: "Owner", status: "Active" },
    { name: "Julian Reyes", email: "julian@fireandslice.co", role: "Manager", status: "Active" },
    { name: "Carlos Mendez", email: "carlos@fireandslice.co", role: "Driver", status: "Active" },
    { name: "Amelia Chen", email: "amelia@fireandslice.co", role: "Driver", status: "Pending Invite" }
  ]);
  ...
  const handleInvite = (e: React.FormEvent) => {
    ...
    setTeam((prev) => [
      ...prev,
      { name: inviteName, email: inviteEmail, role: inviteRole, status: "Pending Invite" }
    ]);
    setInviteEmail("");
    setInviteName("");
  };
  ...
  return (
    <div className="p-6 space-y-8 max-w-5xl">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-brand-navy">Team Management</h1>
        <p className="text-xs text-brand-slate font-medium">Configure roles and zoning dashboard access permissions for your food truck fleet staff.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 p-5 sm:p-6 rounded-3xl bg-white border border-brand-navy/5 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {team.map((member) => (
              ...
              >
                <div>
                  <h4 className="font-extrabold text-brand-navy">{member.name}</h4>
                  <span className="text-[10px] text-brand-slate block font-mono">{member.email}</span>
                </div>

                <div className="flex items-center space-x-4">
                  <div>
                    <span className="block text-[8px] font-bold text-brand-slate uppercase font-mono text-right">ROLE</span>
                    <span className="text-brand-navy font-bold">{member.role}</span>
                  </div>
                  <div>
                    <span className="block text-[8px] font-bold text-brand-slate uppercase font-mono text-right">STATUS</span>
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold block ${
                      member.status === "Active" ? "text-emerald-500 bg-emerald-500/10" : "text-amber-500 bg-amber-500/10"
                    }`}>
                      {member.status}
                    </span>
                  </div>
        ...
        <div className="lg:col-span-4 p-5 sm:p-6 rounded-3xl bg-white border border-brand-navy/5 shadow-xl space-y-5">
          ...
          <form onSubmit={handleInvite} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="block text-[9px] font-bold text-brand-slate uppercase font-mono">Operations Role</label>
              <select
                value={inviteRole}
          ...
          <div className="p-3.5 rounded-xl bg-brand-cream/40 border border-brand-navy/5 flex items-start space-x-2 text-[10px] text-brand-slate font-medium">
            ...
            <span>Invited crew members will receive an onboarding link to sync route coordinates and check-in schedules directly on their phones.</span>
          </div>
        </div>
      </div>
    </div>
