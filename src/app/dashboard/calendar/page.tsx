...
import { Calendar as CalendarIcon, Clock, MapPin, ChevronLeft, ChevronRight, PlusCircle, ShieldCheck } from "lucide-react";
...
export default function AvailabilityCalendar() {
  ...
  const calendarDays = Array.from({ length: 30 }, (_, i) => {
    ...
    if ([12, 15, 18].includes(day)) {
      status = "booked";
      bookingRef = day === 12 ? "SR-6092-B14" : day === 15 ? "SR-8104-A9" : "SR-9912-E5";
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-brand-navy">Scheduling Calendar</h1>
          <p className="text-xs text-brand-slate font-medium">Coordinate your food truck active dates, block-out maintenance slots, and review upcoming locations.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 p-5 sm:p-6 rounded-3xl bg-white border border-brand-navy/5 shadow-sm space-y-6">
          <div className="flex items-center justify-between pb-3 border-b border-brand-navy/5">
            <h3 className="text-sm font-extrabold text-brand-navy font-mono uppercase">{currentMonth}</h3>
          ...
          <div className="grid grid-cols-7 gap-2">
            {calendarDays.map((d) => {
              let cellStyle = "bg-white border-brand-navy/5 text-brand-navy hover:border-brand-teal/40";
              if (d.status === "booked") {
                cellStyle = "bg-brand-navy border-brand-navy text-brand-warm-white hover:bg-brand-navy/90";
              } else if (d.status === "blocked") {
                cellStyle = "bg-brand-cream-dark/40 border-brand-navy/5 text-brand-slate line-through";
              }
              ...
              activeDay.status === "booked" ? (
                <div className="space-y-4 text-xs font-medium text-brand-slate">
                  ...
                  <div className="space-y-2.5">
                    <div>
                      <span className="block text-[8px] font-bold text-brand-slate uppercase font-mono">LOCATION</span>
                      <span className="text-sm font-extrabold text-brand-navy block mt-0.5">{activeDay.location}</span>
                    </div>
                    <div>
                      <span className="block text-[8px] font-bold text-brand-slate uppercase font-mono">REFERENCE ID</span>
                      <span className="text-brand-navy font-bold font-mono">{activeDay.bookingRef}</span>
                    </div>
                  </div>
                ...
                <div className="space-y-3 text-xs text-brand-slate font-medium">
                  ...
                  <p className="text-[10px] leading-relaxed">
                    This day is blocked due to scheduled truck maintenance or permit audits. Generators and water valves will undergo checks.
                  </p>
                </div>
              ) : (
                <div className="space-y-4 text-xs text-brand-slate font-medium">
                  ...
                  <p className="text-[10px] leading-relaxed">
                    No active assignments booked. Secure a spot to fill operations schedules.
                  </p>
                  <Link
                    href="/marketplace"
                    className="w-full text-center py-2.5 rounded-full bg-brand-navy hover:bg-brand-teal text-brand-warm-white text-[10px] font-bold transition-all block"
