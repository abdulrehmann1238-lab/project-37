...
import { Compass, Layers, Award, Terminal, ChevronRight } from "lucide-react";

export default function DemoWalkthroughPage() {
  const steps = [
    {
      title: "1. Premium Cinematic Loader",
      desc: "Initially launching the app triggers a vector map drawing animation, pulsing reservation pins, and a percentage counter before scaling/clipping away.",
      icon: <Layers className="w-5 h-5 text-brand-teal" />,
    },
    {
      title: "2. Airbnb-Style Marketplace",
      desc: "Navigate to '/marketplace' to view a searchable, filterable grid of spots coupled with a custom interactive SVG city map that maps hover states automatically.",
      icon: <Compass className="w-5 h-5 text-brand-teal" />,
    },
    {
      title: "3. Progressive Reservation Wizard",
      desc: "Select a spot and click 'Reserve Spot' to launch a 7-step booking wizard with fluid spring sliders and a Stripe checkout simulator.",
      icon: <Terminal className="w-5 h-5 text-brand-teal" />,
    },
    {
      title: "4. Payout Pushing Dashboards",
      desc: "Launch the Truck Owner Portal ('/dashboard') or Admin Panel ('/admin') to check weekly sales trends, occupancy rates, and payout statement items.",
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="space-y-4 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight">
            SpotReserve Demonstration Walkthrough
          </h1>
          <p className="text-sm sm:text-base text-brand-slate font-medium max-w-xl mx-auto leading-relaxed">
            Welcome to the developer walkthrough guide. Follow the instructions
            below to review and verify all 25 pages within this premium SaaS
            demo.
          </p>
        </div>
        ...
        <div className="p-5 rounded-3xl bg-brand-navy text-brand-warm-white border border-white/10 shadow-xl space-y-4 relative">
          ...
          <p className="text-xs text-slate-300 font-medium leading-relaxed">
            Look for the floating **"Demo Control Panel"** button in the
            bottom-right corner of your viewport. Clicking it expands a
            directory detailing all **25 requested pages**. You can click on any
            page to teleport instantly, bypassing manual clicks!
          </p>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {steps.map((step, idx) => (
              <div
                key={idx}
                className="p-6 rounded-2xl bg-white border border-brand-navy/5 shadow-sm space-y-3.5 flex flex-col justify-between"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs text-brand-slate font-semibold leading-relaxed">
            <div className="space-y-2">
              <span className="text-brand-navy font-bold">
                1. Vector Maps (60 FPS Performance)
              </span>
              <p className="font-medium text-[11px]">
                To maintain a buttery smooth 60 FPS scroll and avoid heavy
                WebGL/Mapbox API key requirements, we built coordinates grids
                using raw inline SVGs that map hovered lists, keeping CPU usage
                near zero.
              </p>
            </div>
            <div className="space-y-2">
              ...
              <p className="font-medium text-[11px]">
                Structured with full dynamic routing paths and TypeScript types
                verification. Route params are fully resolved, and Tailwind CSS
                color tokens are custom defined.
              </p>
            </div>
          </div>
        </div>
